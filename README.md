...
- **Dashboard stats cards** — clickable Pending / Approved / Rejected tiles on the validator dashboard. Click a tile to instantly filter the table to that status.
- **Role-based router guards** — navigating to the wrong role's route automatically redirects to the correct interface, keeping requesters and validators in their own spaces.
- **Conflict detection (HTTP 409)** — the API rejects attempts to update a request that has already been approved or rejected, preventing accidental state corruption.
- **Auto-computed duration** — the request list shows the number of vacation days, derived from start/end dates without storing redundant data.
- **Idempotent seed script** — `npm run seed` can be run repeatedly without duplicating users or requests, simplifying local development resets.
